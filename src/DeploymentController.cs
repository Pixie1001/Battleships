...
namespace battleship {
    class DeploymentController {
        ...
        private GameResources _gameResources = new GameResources();
        ...
        private void DoDeployClick() {
            ...
            int row, col;
            row = Convert.ToInt32(Math.Floor((mouse.Y - FIELD_TOP) / (double)(CELL_HEIGHT + CELL_GAP))); // Did this fix it?
            ...
            if (row >= 0 && row < HumanPlayer.PlayerGrid.Height) {
                if (col >= 0 && col < HumanPlayer.PlayerGrid.Width) {
                    ...
                    try {
                        HumanPlayer.PlayerGrid.MoveShip(row, col, _selectedShip, _currentDirection); // Bookmark - ship is being placed wrong. When set to updown, should place from bottom of ship
                    }
                    catch (Exception ex) {
